...
{strip}
	...
	<div>
		<select name="{$FIELD_MODEL->getFieldName()}" class="select2 form-control" tabindex="{$FIELD_MODEL->get('tabindex')}"
			...
			{foreach item=VALUE key=KEY from=$PICKLIST_VALUES}
				<option value="{\App\Purifier::encodeHtml($KEY)}"
						title="{\App\Purifier::encodeHtml($VALUE['name'])}"
						{if $FIELD_VALUE eq $KEY}selected{/if}>{\App\Purifier::encodeHtml($VALUE['name'])}</option>
			{/foreach}
		</select>
	</div>
	<!-- /tpl-Base-Edit-Field-Smtp -->
