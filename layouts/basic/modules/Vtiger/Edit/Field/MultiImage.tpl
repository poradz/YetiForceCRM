...
{strip}
	...
	<div class="border rounded px-2 pt-2 clearfix c-multi-image js-multi-image">
		<input name="{$FIELD_MODEL->getFieldName()}_temp[]" type="file" class="d-none js-multi-image__file"
			   tabindex="{if $FIELD_MODEL->get('tabindex') neq 0}{$FIELD_MODEL->get('tabindex') + $TABINDEX_INCREMENT}{elseif isset($HIGHEST_TABINDEX)}{$HIGHEST_TABINDEX + $TABINDEX_INCREMENT}{else}{0 + $TABINDEX_INCREMENT}{/if}" data-js="jQuery-file-upload"
