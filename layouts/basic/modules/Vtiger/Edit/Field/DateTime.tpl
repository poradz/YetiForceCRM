...
{strip}
	...
	<div class="input-group dateTime">
		<input name="{$FIELD_MODEL->getFieldName()}" class=" {if !$FIELD_MODEL->isEditableReadOnly()} dateTimePickerField {/if} form-control"
			   ...
			   data-date-format="{$USER_MODEL->get('date_format')}" type="text" tabindex="{if $FIELD_MODEL->get('tabindex') neq 0}{$FIELD_MODEL->get('tabindex') + $TABINDEX_INCREMENT}{elseif isset($HIGHEST_TABINDEX)}{$HIGHEST_TABINDEX + $TABINDEX_INCREMENT}{else}{0 + $TABINDEX_INCREMENT}{/if}"
