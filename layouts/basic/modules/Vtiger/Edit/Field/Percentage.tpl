...
{strip}
	...
	<div class="tpl-Edit-Field-Percentage input-group {if $WIDTHTYPE eq 'narrow'}input-group-sm{/if}">
		<input id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->getName()}" type="number" tabindex="{if $FIELD_MODEL->get('tabindex') neq 0}{$FIELD_MODEL->get('tabindex') + $TABINDEX_INCREMENT}{elseif isset($HIGHEST_TABINDEX)}{$HIGHEST_TABINDEX + $TABINDEX_INCREMENT}{else}{0 + $TABINDEX_INCREMENT}{/if}" title="{\App\Language::translate($FIELD_MODEL->getFieldLabel(), $MODULE)}" class="input-medium form-control" min="0" max="100" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
